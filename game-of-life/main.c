...
static size_t filled(uint8_t val) { return val != 0; }

static size_t idx(size_t x, size_t y) { return y * W + x; }

static size_t countNeighboursCommon(size_t x, size_t y) {
  size_t res = 0;
...
static size_t countNeighbours(size_t x, size_t y) {
...
static uint8_t calcState(size_t x, size_t y) {
  uint8_t dead = 0;
  uint8_t alive = 1;
