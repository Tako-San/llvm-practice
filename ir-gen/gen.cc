...
auto genFlush(CBM &cbm) {
  auto *funcType = llvm::FunctionType::get(cbm.builder->getVoidTy(), false);
  auto *func = llvm::Function::Create(funcType, llvm::Function::ExternalLinkage,
                                      "flush", cbm.llvmModule);
  return func;
}

auto genPutPixel(CBM &cbm) {
  auto &builder = *cbm.builder;
  auto *funcType = llvm::FunctionType::get(
      builder.getVoidTy(),
      {builder.getInt64Ty(), builder.getInt64Ty(), builder.getInt8Ty()}, false);
  auto *func = llvm::Function::Create(funcType, llvm::Function::ExternalLinkage,
                                      "putPixel", cbm.llvmModule);
  return func;
}

auto genSwap(CBM &cbm) {
...
auto genCountNeighboursCommon(CBM &cbm) {
  auto &context = *cbm.context;
  auto &builder = *cbm.builder;
  auto *llvmModule = cbm.llvmModule;
...
auto genCountNeighbours(CBM &cbm) {
  auto &context = *cbm.context;
  auto &builder = *cbm.builder;
  auto *llvmModule = cbm.llvmModule;
